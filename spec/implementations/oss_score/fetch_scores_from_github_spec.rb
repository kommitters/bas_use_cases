...
RSpec.describe Implementation::FetchScoresFromGithub do
  ...
  let(:mocked_http_response) do
    instance_double(Net::HTTPSuccess, body: { score: 8.9 }.to_json)
  end
  ...
  before do
    allow(mocked_shared_storage).to receive(:read).and_return(
     ...
     allow(mocked_shared_storage).to receive(:write).and_return(
      { success: { scores: [{ page_id: 'abcd-1234', name: 'example-repo', score: 8.9 }] } }
    )
    ...
    allow(Net::HTTP).to receive(:get_response).and_return(mocked_http_response)

    @bot = Implementation::FetchScoresFromGithub.new(options, mocked_shared_storage)
  end

  it 'reads repos, gets scores from API and normalizes response' do
    result = @bot.execute
