...
require_relative '../../../src/utils/warehouse/operaton/incident_formatter'
...
RSpec.describe Implementation::FetchRecordsFromOperaton do
  ...
  let(:options) do
    {
      ...
      method: :get,
      params: { startedAfter: '2025-10-01T00:00:00.000+0000' }
    }
  end
  ...
  describe '#write' do
    ...
    let(:operaton_record_one) { { 'id' => 'incident-1' } }
    let(:operaton_record_two) { { 'id' => 'incident-2' } }
    let(:formatted_record_one) { { external_id: 'incident-1' } }
    let(:formatted_record_two) { { external_id: 'incident-2' } }
    let(:formatter_one) { instance_double(Utils::Warehouse::Operaton::Formatter::IncidentFormatter) }
    let(:formatter_two) { instance_double(Utils::Warehouse::Operaton::Formatter::IncidentFormatter) }

    before do
      allow(Utils::Operaton::Request).to receive(:execute).and_return(api_response)
      allow(Utils::Warehouse::Operaton::Formatter::IncidentFormatter)
    ...
    it 'calls the API and writes formatted records to storage' do
      expect(Utils::Operaton::Request).to receive(:execute).with(
        endpoint: 'history/incident',
        query_params: { first_result: 0, max_results: 100, startedAfter: '2025-10-01T00:00:00.000+0000' },
        method: :get,
        body: nil
      )
    ...
    context 'with pagination' do
      let(:api_response_page_one) { double('HTTParty::Response', success?: true, parsed_response: Array.new(100) { { 'id' => 'incident-page-1' } }) }
      let(:api_response_page_two) { double('HTTParty::Response', success?: true, parsed_response: [{ 'id' => 'incident-page-2' }]) }

      before do
        allow(Utils::Operaton::Request).to receive(:execute).and_return(api_response_page_one, api_response_page_two)
        allow(Utils::Warehouse::Operaton::Formatter::IncidentFormatter).to receive(:new).and_return(formatter_one)
        allow(formatter_one).to receive(:format).and_return(formatted_record_one)
      end

      it 'makes multiple API calls and writes to storage for each page' do
        expect(Utils::Operaton::Request).to receive(:execute).with(hash_including(query_params: hash_including(
          first_result: 0, max_results: 100
        ))).ordered
        expect(Utils::Operaton::Request).to receive(:execute).with(hash_including(query_params: hash_including(
          first_result: 100, max_results: 100
        ))).ordered
